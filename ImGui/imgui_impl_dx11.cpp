// dear imgui: Gui Backend for DirectX11
// This needs to be used along with a Platform Backend (e.g. Win32)

// Implemented features:
//  [X] Gui: User texture binding. Use 'ID3D11ShaderResourceView*' as ImTextureID. Read the FAQ about ImTextureID!
//  [X] Gui: Large meshes support (64k+ vertices) even with 16-bit indices (ImGuiBackendFlags_GuiHasVtxOffset).
//  [X] Gui: Expose selected render state for draw callbacks to use. Access in '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Gui_RenderState'.
...
//  2024-10-07: DirectX11: Expose selected render state in ImGui_ImplDX11_RenderState, which you can access in 'void* platform_io.Gui_RenderState' during draw callbacks.
...
//  2019-05-29: DirectX11: Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_GuiHasVtxOffset flag.
//  2019-04-30: DirectX11: Added support for special ImDrawCallback_ResetRenderState callback to reset render state.
//  2018-12-03: Misc: Added #pragma comment statement to automatically link with d3dcompiler.lib when using D3DCompile().
//  2018-11-30: Misc: Setting up io.BackendGuiName so it can be displayed in the About Window.
...
// Backend data stored in io.BackendGuiUserData to allow support for multiple Dear ImGui contexts
// It is STRONGLY preferred that you use docking branch with multi-viewports (== single Dear ImGui context + multiple windows) instead of multiple Dear ImGui contexts.
static ImGui_ImplDX11_Data* ImGui_ImplDX11_GetBackendData()
{
    return ImGui::GetCurrentContext() ? (ImGui_ImplDX11_Data*)ImGui::GetIO().BackendGuiUserData : nullptr;
}
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    platform_io.Gui_RenderState = &render_state;
    ...
    platform_io.Gui_RenderState = nullptr;
...
bool    ImGui_ImplDX11_Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
    ImGuiIO& io = ImGui::GetIO();
    IMGUI_CHECKVERSION();
    IM_ASSERT(io.BackendGuiUserData == nullptr && "Already initialized a Gui backend!");

    // Setup backend capabilities flags
    ImGui_ImplDX11_Data* bd = IM_NEW(ImGui_ImplDX11_Data)();
    io.BackendGuiUserData = (void*)bd;
    io.BackendGuiName = "imgui_impl_dx11";
    io.BackendFlags |= ImGuiBackendFlags_GuiHasVtxOffset;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.
...
void ImGui_ImplDX11_Shutdown()
{
    ...
    io.BackendGuiName = nullptr;
    io.BackendGuiUserData = nullptr;
    io.BackendFlags &= ~ImGuiBackendFlags_GuiHasVtxOffset;
    IM_DELETE(bd);
}
